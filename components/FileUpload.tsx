...
export const FileUpload: React.FC<FileUploadProps> = ({ onDataLoaded, onCompareLoaded }) => {
  ...
  return (
    <div className="max-w-xl mx-auto mt-10">
      ...
      {/* Tabs */}
      <div className="flex bg-white rounded-t-xl overflow-hidden border-b border-slate-200">
        <button 
            onClick={() => setMode('single')}
            className={`flex-1 py-3 text-sm font-bold transition ${mode === 'single' ? 'bg-blue-50 text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:bg-slate-50'}`}
        >
            Tek Rapor Yükle
        </button>
        <button 
            onClick={() => setMode('compare')}
            className={`flex-1 py-3 text-sm font-bold transition ${mode === 'compare' ? 'bg-blue-50 text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:bg-slate-50'}`}
        >
            Rapor Karşılaştır
        </button>
      </div>

      <div className="bg-white p-6 rounded-b-xl shadow-lg border border-slate-100">
        ...
        {mode === 'single' ? (
            ...
            <div className="space-y-6">
                <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-800">Rapor Fark Analizi</h2>
                    <p className="text-slate-500 mt-2">Dün ve bugünün raporunu yükleyerek aradaki farkları (Gelen/Giden/Değişen) görün.</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                    <div className="border border-slate-200 rounded-lg p-4 bg-slate-50 hover:bg-white transition-colors">
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Eski Rapor (Dün)</label>
                        <div className="relative">
                            ...
                            />
                            <div className="flex items-center gap-2 p-3 bg-white border border-slate-300 rounded text-sm text-slate-600">
                    ...
                    <div className="border border-slate-200 rounded-lg p-4 bg-slate-50 hover:bg-white transition-colors">
                        <label className="block text-xs font-bold text-blue-500 uppercase mb-2">Yeni Rapor (Bugün)</label>
                        <div className="relative">
                             <input 
                                type="file" 
                                accept=".xlsx"
                            ...
                            />
                            <div className="flex items-center gap-2 p-3 bg-white border border-blue-300 rounded text-sm text-slate-600 shadow-sm ring-1 ring-blue-100">
